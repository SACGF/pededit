...
#
# Bundles the dev render harness with esbuild, runs it to emit
# test-output/dev-*.svg, then screenshots each SVG in headless Chromium.
...
#
# Usage: ./render-u.sh [MAXPX, default 1800] [name filter, e.g. synth]
set -e
cd "$(dirname "$0")"
...
for f in test-output/dev-*"$FILTER"*.svg; do
  [ -e "$f" ] || continue
